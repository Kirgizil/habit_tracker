...
month_trans = ['01-31', '02-28', '03-31', '04-30', '05-31', '06-30', '07-31',
               '08-31', '09-30', '10-31', '11-30', '12-31']


month_leap = ['01-31', '02-29', '03-31', '04-30', '05-31', '06-30', '07-31',
              '08-31', '09-30', '10-31', '11-30', '12-31'] 
...
analytics = {
    ...
    '3': 'See The Longest Streaks Of All Currently Tracked Habits',
    '4': 'See The Longest Streak Of A Specific Habit',
    '5': 'See The Streak Average Of All Currently Tracked Habits',
    '6': ('See The Total Number Of Started Streaks Of All Currently Tracked '
          'Habits'),
    '7': 'See All-Time Streak Performance Score',
    '8': 'See Monthly Performance Scores',
    '9': 'See Monthly Performance Scores For One Year'
...
def calc_func(i, time_stamp, d_today, d_current, d_other, w_current, w_today,
              w_other, w_soon):
    '''
    Takes as input the current time stamp, a list of record time stamps for
    one habit containing the name, periodicity and ID of the habit, and a 
    range of lists in which data will be sorted/written into per habit (habit
    name, last recorded time stamp, longest habit streak and its unit (days,
    weeks)).
    
    Parameters
    ----------
    i : list
        Containing record time stamps, as well as name, periodicity and ID for
        one habit.
    time_stamp : str
        Current time stamp automatically assigend during login using the ISO 
        format 'YYYY-MM-DD HH:MM:SS'.
    d_today : list
        Containing daily habit name, last recorded time stamp, longest habit 
    ...
    w_current : list
        Containing weekly habit name, last recorded time stamp, longest habit 
    ...
    ## periodicity) and sort obtained list by date
    temp_list = list(set(i[1:]))
    
    temp_list.sort(key=itemgetter(0), reverse=True)
    
    ## Calculate current streak for all daily habits which contain records
    if i[0][1] == 'd' and temp_list:

        days = [time_stamp.split(' ')[0]]

        ## generate a list of dates until first habit record
        while not list(set([temp_list[-1][0]]) & set(days)):

            days.extend(month_day_func(days[-1], 1))
    ...
    ## Calculate current streak for all weekly habits which contain records
    elif i[0][1] == 'w' and temp_list:
      
        weeks = [[time_stamp.split(' ')[0]]]

        ## generate a list of weeks until first habit record
        while not list(set([temp_list[-1][0]]) & 
                       set([item for sublist in weeks for item in sublist])):

            weeks.extend([month_day_func(weeks[-1][-1], 7)])

        records = [i[0] for i in temp_list]

        
        if list(set(weeks[0]) & set(records)):
        ...
        if records[0] == weeks[1][-1]:
            
            w_current = display_analytics(
        ...
        elif records[0] == weeks[0][0]:
            
            w_today = display_analytics(
        ...
        elif records[0] in weeks[1][:-1]:
            
            w_soon = display_analytics(
...
def display_analytics(display_list, i, temp_list, current_streak=None, 
                      unit=None):
    '''
    This function produces a list of key parameter per habit used in the 
    definition calc_func (habit name, last recorded time stamp, current 
    streak, streak unit).
    
    Parameters
    ----------
    display_list : list
        Contains habit name, last recorded time stamp, current streak, 
        streak unit per habit.
    i : list
        Containing record time stamps, as well as name, periodicity and ID for
        one habit.
    temp_list : list
        Sorted list of all time stamps recorded per habit.
    ...
    display_list.append(
        ...
        [
            ...
            color.BOLD + color.UNDERLINE + i[0][0] + color.END,
            temp_list[0][0],
            temp_list[0][1],
            current_streak,
            unit,
...
def month_day_func(time_stamp, number):
    '''
    Produces a list of dates counting down daily from current time stamp. The 
    user defines the length of the list. Leap years are taken into account.

    Parameters
    ----------
    time_stamp : str
        Current time stamp (date format YYYY-MM-DD is also accepted) assigned 
    ...
    -------
    increment : list
        A list of dates 'YYYY-MM-DD' with a daily increment starting one day 
        after the current time stamp.

    '''
    ## define day, month and year from where to count back daily
    day = time_stamp.split(' ')[0].split('-')[2]
    month = time_stamp.split(' ')[0].split('-')[1]
    year = time_stamp.split(' ')[0].split('-')[0]

    ## differentiate between normals and leap years
    month_day_back = {
    ...
    for i in range(number):
        ...
        elif (day == '01' and month != '01' and int(year)%4 == 0 and 
              year[2:] != '00'):
            
            day = month_day_back[month][1]
            month = str(int(month) - 1).zfill(2)
...
def weekly_analytics(i, zahl, option):
    '''
    Calculates length of streaks per weekly habit for all recorded data.
    ...
    option : str
        decision between calculating and prinitng the longest or the average 
        streak per habit.
    ...
    if len(i) == 1 and i[0][1] == 'w':
        ...
        streak, streaks, prev = 0, [], []
        
        month_day = temp_list[0][0][5:]
        year = temp_list[0][0].split('-')[0]
        month = temp_list[0][0].split('-')[1]
        day = temp_list[0][0].split('-')[2]
        ...
        while len(temp_list) > 0:
            ...
            if list(set([temp_list[0][0]]) & set(week)):
            ...
            elif list(set([temp_list[0][0]]) & set(prev)):
        ...
        if option == 'max':
            ...
            if max(streaks) > 1 or max(streaks) == 0:
                print(
                    ...
                    ' {} --> {:3} weeks'
                    .format(i[0][0].ljust(zahl), max(streaks))
        ...
        ## Number of streaks per habit is printed to the screen
        elif option == 'num':
        ...
        ## performance score
            ...
            w_months.append(round((max(streaks)/len(streaks)), 1))
    
    return test_value, w_months ## for unittesting


def daily_analytics(i, zahl, option):
    '''
    Calculates length of streaks per daily habit for all recorded data.
    ...
    if i[0][1] == 'd':
        ...
        for date in temp_list:
            ...
            if (
                ...
                temp = date[0]
                    ...
                    int(date[0].split('-')[1]) ==
                    int(temp.split('-')[1]) + 1 and
                    date[0][5:] in month_trans and
                    int(date[0][:4])%4 != 0
                ...
                temp = date[0]
                    ...
                    int(date[0].split('-')[1]) ==
                    int(temp.split('-')[1]) + 1 and
                    date[0][5:] in month_leap and
                    int(date[0][:4])%4 == 0 and
                    date[0][2:4] != '00'
        ...
        ## Average streak per habit is printed to the screen
            ...
            print(
                ...
                ' {} --> {:5} days'
                .format(i[0][0].ljust(zahl), round(sum(streaks)/len(streaks),1))
        ...
        ## Number of streaks per habit is printed to the screen
            ...
            print(
                ...
                ' {} --> {:3} streaks'
                .format(i[0][0].ljust(zahl), len(streaks))
        ...
        ## performance score
            ...
            d_months.append(round((max(streaks)/len(streaks)), 1))

    return test_value, d_months


def week_analytics(month_day, year, month, day):
    '''
    Generates a list of seven dates 'YYYY-MM-DD' in daily increments (1 week) 
    ascending. Leap years are taken into account.

    Parameters
    ----------
    month_day : str
        'MM-DD' string of the result list starting date minus 1 day.
    year : str
        'YYYY' string of the result list starting date minus 1 day.
    month : str
        'MM' string of the result list starting date minus 1 day.
    day : str
        'DD' string of the result list starting date minus 1 day.
    ...
    for z in range(7):
        ...
        if (month_day not in month_trans and
            int(year)%4 != 0):
        ...
        elif (month_day not in month_leap and
              int(year)%4 == 0 and
              year[2:] != '00'):
        ...
        elif (month_day not in month_trans and
              int(year)%400 == 0 and
              year[2:] == '00'):
...
def decision_process(name):
    '''
    Streamlines nested navigation for the first two decision points of the 
    application.
...
def habit_registration(period, db_name, db_habit_table, user_id, text=''):
    '''
    ...
    ## handle invalid input
    while habit_periodicity not in period.keys():
...
def unique_func(lookup, db_name, db_table, name, domain):
    '''
    Streamlines functionality to only allow unique user- and habit names in 
    the database.
...
#    dbm.db_record_table_creation("productive.db", "record_table", "habit_table")
    ...
    ## global variable time stamp and allowed input formats
...
#    sys.exit()
    ...
    code = (int(decision1), int(decision2))
    ...
    if code == (1, 1):
        ...
        if habit_id == 'x':
            ...
            if retrospective == 'r':
                ...
                ## handle invalid input (time stamp format)
                ...
                ## Generate a 2 weeks list of dates in daily increments 
                ## starting with current time stamp
                fourteen_day_increment = month_day_func(time_stamp, 14)
                
                fourteen_day_increment.insert(0, time_stamp.split(' ')[0])
                
                ## handle invalid input (last fourteen days)
        ...
        dates.extend(month_day_func(time_stamp, 14))
        dates = tuple(dates)
        ...
        print(color.BOLD + color.UNDERLINE + 
              '\n\n All Currently Tracked Habits:' + color.END + '\n')
        ...
        ## handle invalid input
        while periodicity not in period:
            
            periodicity = input(
                
                '\n\n Invalid Input. You Must Choose Between d=daily And w=weekly --> '
        ...
        print(
            ...
            'List Of {} Habits:'.format(period[periodicity].upper()) + 
            color.END + '\n'
...
#                    dates=('2021-01', '2021-02', '2021-03')
        ...
        print(
            ...
            color.UNDERLINE + color.BOLD + '\n\n Current Habits Longest Streak'
            ':\n' + color.END
            
            )
            
        ## calculate the longest habit name (printing to screen purpose)
        zahl = len(max([i[0][0] for i in masterlist]))
        
        ## generate longest streak data for currently tracked habits
        for i in masterlist:
            
            daily_analytics(i, zahl, 'max')
            weekly_analytics(i, zahl, 'max')
        ...
        print(
            ...
            '\n\n ' + color.UNDERLINE + color.BOLD + 'Longest Streak For...'
            '\n' + color.END
        ...
        for i in masterlist:
            ...
            daily_analytics(i, zahl, 'ave')
            weekly_analytics(i, zahl, 'ave')
        ...
        for i in masterlist:
            ...
            daily_analytics(i, zahl, 'num')
            weekly_analytics(i, zahl, 'num')
        ...
        for i in masterlist:
            ...
            daily_analytics(i, zahl, 'ratio')
            weekly_analytics(i, zahl, 'ratio')
        ...
        print(
            ...
            .format(number_to_month[yyyy_mm[-2:]], yyyy_mm[:4]) + color.END
        ...
        for i in range(1, 13):
            ...
            for i in masterlist:
                ...
                if daily_analytics(i, zahl, 'year')[1]:
                    
                    months.append(daily_analytics(i, zahl, 'year')[1][0])
                
                elif weekly_analytics(i, zahl, 'year')[1]:
                
                    months.append(weekly_analytics(i, zahl, 'year')[1][0])
        ...
        h_zahl = max([len(i) for i in names_habits])
